...
def _log_overlap(args, work, target, t):
    ...
    if not target.startswith("driver/"):
        return
    res = _git(work, "for-each-ref", "--format=%(refname:short)", "refs/heads/driver/")
    others = [b for b in res.stdout.split() if b != target]
    if not others:
        return
    for other in others:
...
def selftest_land():
    ...
    print("ok: (q) an overlapping landing is recorded and still lands")
