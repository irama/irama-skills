...
3. `git switch <default-branch>` (in the main checkout) and `git pull --ff-only`. Leave the checkout clean and on `<default-branch>`: step 4c detaches it, lands, and returns it there.
4. **Merge one branch at a time, oldest-committed first** (a single branch for `/merge`):
   ...
   b) **Review the branch BEFORE it lands, never after.** Capture `pre=$(git rev-parse <default-branch>)`, run the step-5 Codex review with `--base "$pre"`, and fix any finding on the branch. A review that runs after the merge can only ask you to back code out of `<default-branch>`, which is the failure this order removes.
   c) **Land it with the transactional primitive. Never `git switch <default-branch> && git merge`.**

      ```
      python3 <skill-dir>/../driver/driver_state.py land \
        --work "$(git rev-parse --show-toplevel)" --target "<default-branch>" \
        --source "<branch>" --wait 1800 \
        --gate "npm run typecheck && npm test"
      ```

      It merges on a detached HEAD, runs the gate there, and moves `<default-branch>` only when the gate is green **and** the branch has not moved since the gate ran. Exit codes: **0** landed (it prints the commit), **3** conflict (resolve per the playbook below; never force or discard), **4** gate red — `<default-branch>` is untouched, fix on the branch and re-run, **5** another thread landed during the gate, so re-run and let the gate see the new base, **6** the checkout is dirty or `<default-branch>` is checked out in another worktree.
   d) **What goes in `--gate`:** typecheck + tests (`npm run typecheck && npm test`, or the repo equivalent), per merged branch. Gating each merge separately is what isolates a bad branch.

      **Drop the gate to a bare `true` ONLY when both hold: (i) the merge is a true fast-forward** — `git merge-base --is-ancestor <default-branch> <branch>` is true, so the merged tree is byte-identical to the branch tip — **and (ii) that tip was gated by a real `/commit` pass** (typecheck + tests + lint green on that exact tree). Then the gate would validate the same tree twice, and every gate command is a full-context API request (§ Session length is the cost driver).
...
5. **Codex review gate (second-model axis — runs here, not at `/push`).** These are the rules step 4b runs by, on the branch, **before** it lands. **Quota discipline first** (2026-07-18: a review loop burned ~5 full-diff passes in one merge):
   ...
   **If Codex cannot run** (usage limit — it's on a ChatGPT subscription and this happens for days at a time — auth failure, offline, or a Codex error), fall back to the **`adversarial-reviewer`** subagent (Opus). Report **which reviewer actually ran** — never imply the Codex gate passed when it did not run. A completed fallback review closes the gate: record the reviewer's name in the row, do not call it degraded, do not carry it forward for `/push` to re-check, and never offer a later Codex re-run of it.
